...
async function ludwigLiveCheck(client:CustomClient) {
    try {
        ...
        if (today.getUTCHours() <= 11 || today.getUTCHours() >= 20) { //Between Noon and 3am PST
            ...
            const canonicalURL:string = parse(await (await fetch(`https://www.youtube.com/@ludwig/live`))?.text())?.querySelector('link[rel=canonical]')?.getAttribute('href') || "";
            c.live = false
            c.configured = canonicalURL?.includes('/watch?v=') || false;
            if (c.configured) {
                c.vid = canonicalURL.match(/(?<==).*/)![0]

                const data:any = await fetch(youtubeURLbase + c.vid).then(response => response.json())
                if (data.error) {
                    console.error(data)
                    process.exit(1)
                }
                ...
                if (c.live) {
                    ...
                    if (lastTitle != c.title) {
                        console.log(c);
                        lastTitle = c.title;
                        (<TextChannel>client.channels.cache.get(notifChannelID)).send({
                            embeds: [
                                {
                                    color: Colors.Red,
